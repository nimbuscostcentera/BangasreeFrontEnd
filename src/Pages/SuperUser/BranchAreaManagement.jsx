...
import { getAreaList, ClearState5 } from "../../Slice/Area/AreaListSlice";
import {
  BranchCreatefunc,
  ClearState36,
...
export default function BranchAreaManagement() {
  ...
  // AreaList
  const { isloading5, areaList, error5, isError5, isSuccess5 } = useSelector(
  ...
  // BranchList
  const { isloading35, Resp35, error35, isError35, isSuccess35 } = useSelector(
    (state) => state.branch
  );
  ...
  useEffect(() => {
    dispatch(getAreaList({ ...global, ...Filters2 }))
      .then((res) => {
        if (res.payload) {
          setArea(res.payload);
          let areaList = res.payload;
          let actArea = areaList.filter((i) => i?.Status != 0);
          setActiveArea(actArea);
        }
      })
      .catch((err) => {
        console.log(err);
  ...
  function OnEditBranch(e) {
    ...
    let a = { ...global, ...finalObj, BranchId: ids[0] };
