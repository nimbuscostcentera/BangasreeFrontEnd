...
import { Divider, Typography, Alert, AlertTitle, Stack } from "@mui/material";
...
export default function SuperUserManagement() {
  ...
  const ApprovedStatusHandler = () => {
    ...
    if (check) {
      ...
      //setWarMsg("You Can not Active or Inactive yourself");
    } else {
      dispatch(ClearState40());
  ...
  const OnEditView = () => {
    if (ids.length > 1) {
      ...
      // setalert(true);
      // setWarMsg(" Select 1 Backoffice User to View and Edit.");
    } else {
      var a = ids[0];
      // var data = SuperUserDetails.filter((item) => item?.SuperUserID === a);
